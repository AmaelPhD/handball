...
f[1, 1] = buttongrid1 = GridLayout(tellwidth = false) #creating two lines of buttons one for scored shot and one for missed shot
f[1, 3] = buttongrid2 = GridLayout(tellwidth = false) #creating two lines of buttons one for scored shot and one for missed shot
...
buttons11 = buttongrid1[1:3, 1] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3]
buttons12 = buttongrid1[1:3, 2] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3]
buttons13 = buttongrid1[1:3, 3] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3] #button for scored then for missed
buttons21 = buttongrid2[1:3, 1] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3]
buttons22 = buttongrid2[1:3, 2] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3]
buttons23 = buttongrid2[1:3, 3] = [Button(f, label = "",buttoncolor=:transparent,width=180,height=200) for l in 1:3]


for i in 1:3 #clicking on button update the count
    on(buttons11[i].clicks) do n
        counts1[][i] += 1
        notify(counts1)
    end
end
for i in 1:3 #clicking on button update the count
    on(buttons12[i].clicks) do n
        counts1[][i+3] += 1
        notify(counts1)
    end
end
for i in 1:3 #clicking on button update the count
    on(buttons13[i].clicks) do n
        counts1[][i+6] += 1
        notify(counts1)
    end
end
for i in 1:3
    on(buttons21[i].clicks) do n
        counts2[][i] += 1
        notify(counts2)
    end
end
for i in 1:3
    on(buttons22[i].clicks) do n
        counts2[][i+3] += 1
        notify(counts2)
    end
end
for i in 1:3
    on(buttons23[i].clicks) do n
        counts2[][i+6] += 1
        notify(counts2)
    end
end
